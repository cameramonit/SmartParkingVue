...
const confirmDelBatch = () => {
  ...
  request.post('/suggestion/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 删除
const del = (id) => {
  request.delete('/suggestion/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('suggestion.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
