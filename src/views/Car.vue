...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/car/export`)
}
...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('car.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
        </el-icon>  <span style="vertical-align: middle"> 新增 </span>
      </el-button>
      <el-upload
          v-if="auths.includes('car.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/car/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('car.export')">
        <el-icon style="vertical-align: middle">
          <Top />
    ...
    <div style="margin: 10px 0">
      <el-table :data="state.tableData" stripe border  @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="cid" label="车辆唯一id"></el-table-column>
    ...
    <el-dialog v-model="dialogFormVisible" title="车辆管理信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        ...
        <el-form-item prop="carNumber" label="车辆名">
          <el-input v-model="state.form.carNumber" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item prop="uid" label="车主">
