...
const confirmDelBatch = () => {
  ...
  request.post('/parking/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 删除
const del = (id) => {
  request.delete('/parking/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('parking.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column prop="pid" label="停车场唯一id"></el-table-column>
      <el-table-column prop="name" label="名称"></el-table-column>
      <el-table-column prop="parkingSpaceNumber" label="车位数量"></el-table-column>
      ...
      <el-table-column prop="unitCost" label="单元收费"></el-table-column>
        ...
        <el-table-column label="操作" width="180">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('parking.edit')">编辑</el-button>
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('parking.delete')">
              <template #reference>
