...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/record/export`)
}
...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-upload
          v-if="auths.includes('record.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/record/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('record.export')">
        <el-icon style="vertical-align: middle">
          <Top />
    ...
    <el-dialog v-model="dialogFormVisible" title="停车记录信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        <el-form-item prop="rid" label="记录唯一">
          <el-input v-model="state.form.rid" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item prop="carName" label="车辆名称">
          <el-input v-model="state.form.carName" autocomplete="off"></el-input>
        </el-form-item>
